 ...
 */
		...
		if ($vo->situacao == dominioSituacaoDemanda::$CD_SITUACAO_DEMANDA_FECHADA) {
			...
			if($vocontratoDemanda != null){
				...
				if(!$temAmbosDocsAExibir){
					throw new excecaoGenerica("Fechamento sѓ permitido quando ambos os documentos 'MINUTA' (em word) e 'CONTRATO' (em pdf) forem anexados р demanda.", null, $vocontratoDemanda);
				}
				...
				if(isColecaoVazia($retorno)){
					throw new excecaoConsultaVazia("Fechamento da demanda permitido apenas para contrato incluэdo na (1)planilha e na funчуo (2)'informaчѕes adicionais'.");
				}*/
	...
	function existeDemandaAbertaContratoProrrogacao($voDemanda) {
		...
		if(!$isSituacaoFechada && $vocontrato != null){
			...
			if($retorno){
				throw new excecaoGenerica("Jс existe uma demanda ABERTA para o Contrato selecionado. Verifique se o TERMO/ADITIVO indicado estс correto.");
			}
		}
