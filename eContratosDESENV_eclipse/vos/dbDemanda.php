...
class dbDemanda extends dbprocesso {
	...
	function consultarPorChaveTelaJoinContrato($vo, $isHistorico, $isConsultaPorChave, $isTrazerMaisDadosContrato=false) {
		...
		//o join com group by щ para o caso de haver 2 ou mais empresas para o mesmo doc (O QUE NAO PODE ACONTECER) 
 ...
 */
	...
	static function validarDadosContratada($voDemanda, $vocontratoDemanda){
		...
		if($isContratoAssinadoSEI && $isCaracteristicaInvalida){
			throw new excecaoGenerica("Fechamento nуo permitido: preencha os campos da contratada em questуo, na funcao '".vopessoa::getTituloJSP()."', que se referem р assinatura no SEI|" 
					. $vopessoa->toString() . ".");
		}			
	}	
		...
		if(isDemandaContratoModificacaoObrigatorio($voDemanda)){		
			...
			if(!isColecaoVazia($colecao)){
				...
				if($vlGlobalmod != $vlGlobal){
					throw new excecaoGenerica("Valor GLOBAL do contrato ('$vlGlobal') difere do calculado na funчуo '".voContratoModificacao::getTituloJSP()
	...
	function validarAlteracao($vo) {
		...
		if ($isSituacaoNovaFechada) {
			...
			//se a situacao ja foi fechada um dia, quer dizer que ela щ antiga e nao precisa passar pela nova validacao, sob pena de inviabilizar as alteracoes
			if (!$naoValidarFechamento) {
				...
				if($isDemandaTipoContrato && $vocontratoDemanda != null){
					...
					if(!isDemandaContratoAssinado($vo)){
						throw new excecaoGenerica("Fechamento nуo permitido: indique a FASE em que o termo foi assinado (se fэsico, digital ou pelo SEI...). |"
	...
	function existeDemandaAbertaContratoProrrogacao($voDemanda) {
		...
		if(!$isSituacaoFechada && $vocontrato != null){
			...
			if(!$permiteDemandaMesmoContrato){
				...
				//$vocontrato = new vocontrato();			
				if($retorno){
					throw new excecaoGenerica("Jс existe uma demanda ABERTA para o Contrato ".$vocontrato->getCodigoContratoFormatado()
							.", que deve ser utilizada. Verifique se o TERMO/ADITIVO indicado estс correto.");
				}
			}
		}
