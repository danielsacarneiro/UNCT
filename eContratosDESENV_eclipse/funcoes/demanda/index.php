    ...
    <TABLE id="table_filtro" class="filtro" cellpadding="0" cellspacing="0">
        <TBODY>
	        ...
	        <TR>
			  ...
			  Título:<INPUT type="text" id="<?=voDemanda::$nmAtrTexto?>" name="<?=voDemanda::$nmAtrTexto?>" value="<?=$filtro->vodemanda->texto?>"  class="camponaoobrigatorio" size="15">
			  </TD>			  
	            ...
	            <TD class="campoformulario">				
	            <INPUT type="text" onkeyup="formatarCampoPRT(this, event);" id="<?=voDemandaTramitacao::$nmAtrProtocolo?>" name="<?=voDemandaTramitacao::$nmAtrProtocolo?>" value="<?php echo($filtro->vodemanda->prt);?>" class="camponaoobrigatorio" size="25">
	            <?php 
			...
			<TR>
                ...
                <TD class="campoformulario" width="1%" colspan=3>
                Nome: <INPUT type="text" id="<?=vopessoa::$nmAtrNome?>" name="<?=vopessoa::$nmAtrNome?>"  value="<?php echo($filtro->nmContratada);?>"  class="camponaoobrigatorio" size="25">                
            ...
            <TR>
	            <TH class="campoformulario" nowrap width="1%" ROWSPAN=2>Proc.Licitatório:</TH>
	            <TD class="campoformulario" ROWSPAN=2><?php getCampoDadosProcLicitatorioComCPL($filtro->voproclic);?>
	            </TD>	            
	            <TH class="campoformulario" >Vl.Contrato:</TH>
	            <TD class="campoformulario" ><INPUT type="text" id="<?=filtroManterDemanda::$NmAtrVlGlobalInicial?>" name="<?=filtroManterDemanda::$NmAtrVlGlobalInicial?>"  value="<?php echo($filtro->vlGlobalInicial);?>"
	            							onkeyup="formatarCampoMoedaComSeparadorMilhar(this, 2, event);" class="camponaoobrigatorioalinhadodireita" size="15" <?=getMsgPlaceHolder("Vl.Global")?>>
             ...
             <TBODY>
                ...
                for ($i=0;$i<$tamanho;$i++) {
                	$registro = $colecao[$i];
                        ...
                        $vocontratodematual = $voDemandaContrato->voContrato;
                        if(isContratoValido($vocontratodematual) && $voAtual->tipo == dominioTipoDemanda::$CD_TIPO_DEMANDA_CONTRATO){
                        	$tpTempContrato = dominioTipoContrato::getDescricaoStatic($vocontratodematual->tipo, dominioTipoContrato::getColecaoInstrumentos());
